...
pub fn build_event_parse_prompt(user_text: &str) -> String {
    ...
    format!(
        ...
        Use today's local date ({date_hint}) as the reference for relative dates like 'tomorrow' or 'next week'. \
...
pub fn parse_response_to_event(response: &str) -> anyhow::Result<CalendarEvent> {
    ...
    let parsed: ParsedEvent = serde_json::from_str(&json).map_err(|error| {
        anyhow::anyhow!(
            "Failed to parse extracted event JSON: {error}\nExtracted JSON: {json}\nRaw response: {response}"
        )
    })?;
...
fn extract_event_json(response: &str) -> Option<String> {
    ...
    if serde_json::from_str::<ParsedEvent>(direct).is_ok() {
...
pub fn create_calendar_event(event: &CalendarEvent) -> CalendarDelivery {
    #[cfg(target_os = "windows")]
    {
        match save_to_outlook(event, selected_outlook_calendar().ok().as_ref()) {
            ...
            Err(error) => {
                tracing::warn!(error = %error, "local Outlook calendar save failed; falling back to ICS");
            }
        }
    }
    ...
    match launch_ics(&ics, &event.title) {
        ...
        Err(error) => CalendarDelivery {
            saved_to_calendar: false,
            file_opened: false,
            method: "failed",
            message: format!("Could not save or open calendar event: {error}"),
        },
    }
}
...
fn generate_ics(event: &CalendarEvent) -> String {
    ...
    ics.push_str(&format!("DTSTART:{start}\r\n"));
    ics.push_str(&format!("DTEND:{end}\r\n"));
...
fn format_ics_dt(dt: &str) -> String {
    let digits: String = dt.chars().filter(|ch| ch.is_ascii_digit()).collect();
...
fn launch_ics(ics_content: &str, title: &str) -> anyhow::Result<()> {
...
fn save_to_outlook(
    ...
    let script_path = write_outlook_save_script()?;
    let output = std::process::Command::new("powershell.exe")
        .args(["-NoProfile", "-ExecutionPolicy", "Bypass", "-File"])
        .arg(&script_path)
        .arg(&event.title)
...
#[cfg(target_os = "windows")]
fn write_outlook_save_script() -> anyhow::Result<PathBuf> {
    let script = r#"
param(
...
"#;
    ...
    write_temp_script("aegis_outlook_calendar", script)
}

#[cfg(target_os = "windows")]
fn write_outlook_list_script() -> anyhow::Result<PathBuf> {
    let script = r#"
$outlook = New-Object -ComObject Outlook.Application
$namespace = $outlook.Session
try {
...
"#;
    ...
    write_temp_script("aegis_outlook_calendars", script)
}

#[cfg(target_os = "windows")]
fn write_temp_script(prefix: &str, script: &str) -> anyhow::Result<PathBuf> {
...
fn selected_outlook_calendar() -> anyhow::Result<OutlookCalendarPreference> {
    let contents = std::fs::read_to_string(outlook_calendar_preference_path())?;
