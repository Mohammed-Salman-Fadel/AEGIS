...
impl RagClient {
    ...
    pub async fn init(&self) -> anyhow::Result<()> {
        let resp = self.client.post(&format!("{}/init", self.base_url))
            .send()
            .await?;
    ...
    pub async fn ingest(&self, file_path: String) -> anyhow::Result<()> {
        ...
        let resp = self.client.post(&format!("{}/index", self.base_url))
    ...
    pub async fn retrieve(&self, query: &str, limit: usize) -> anyhow::Result<Vec<String>> {
        ...
        let resp = self.client.post(&format!("{}/query", self.base_url))
            .json(&QueryRequest { query: query.to_string(), top_k: limit })
            .send()
            .await?;
