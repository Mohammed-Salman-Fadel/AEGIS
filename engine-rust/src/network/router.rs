...
async fn handle_pdf_ingest(
    ...
    if tokio::fs::create_dir_all(&state.rag_upload_dir).await.is_err() {
    ...
    while let Ok(Some(field)) = multipart.next_field().await {
        let file_name = field
            ...
            .unwrap_or_else(|| format!("upload-{}.bin", now_epoch_millis()));

        let Ok(data) = field.bytes().await else {
            return StatusCode::BAD_REQUEST;
        };

        let upload_path = state.rag_upload_dir.join(file_name);
        ...
        if state
            .orchestrator
            .rag_client
            .ingest_path(&upload_path.display().to_string())
            .await
            .is_err()
        {
...
fn sanitize_upload_name(name: &str) -> String {
    ...
    if sanitized.trim_matches('_').is_empty() {
        format!("upload-{}.bin", now_epoch_millis())
    } else {
        sanitized
    }
}
