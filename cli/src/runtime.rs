...
//! Next TODOs: add richer version compatibility checks and migrate v2 update/rollback state into the same layout.
...
pub const DEFAULT_RAG_HOST: &str = "127.0.0.1";
pub const DEFAULT_RAG_PORT: u16 = 8000;
pub const DEFAULT_OLLAMA_URL: &str = "http://127.0.0.1:11434";
pub const DEFAULT_MODEL: &str = "llama3.2:3b";
pub const DEFAULT_UI_URL: &str = "http://localhost:8080";
pub const DEFAULT_RAG_URL: &str = "http://127.0.0.1:8000";
...
pub struct InstallerManifest {
    ...
    #[serde(default = "default_rag_host_string")]
    pub rag_host: String,
    #[serde(default = "default_rag_port")]
    pub rag_port: u16,
    #[serde(default = "default_rag_url_string")]
    pub rag_url: String,
}
...
impl RuntimeLayout {
    pub fn discover() -> Self {
        ...
        Self {
            ...
            rag_venv_dir: root.join("runtime").join("rag-venv"),
            ...
            rag_pid_path: root.join("run").join("rag.json"),
            root,
        }
    }
    ...
    pub fn installed_rag_dir(&self) -> PathBuf {
        self.current_runtime_dir.join("rag")
    }

    pub fn installed_rag_app_main(&self) -> PathBuf {
        self.installed_rag_dir().join("app").join("main.py")
    }

    pub fn installed_rag_requirements(&self) -> PathBuf {
        self.installed_rag_dir().join("requirements.txt")
    }

    pub fn installed_rag_wheelhouse_dir(&self) -> PathBuf {
        self.installed_rag_dir().join("wheels")
    }

    pub fn bundled_python_exe(&self) -> PathBuf {
        self.current_runtime_dir.join("python").join("python.exe")
    }

    pub fn rag_venv_python_exe(&self) -> PathBuf {
        self.rag_venv_dir.join("Scripts").join("python.exe")
    }
    ...
    pub fn rag_upload_dir(&self) -> PathBuf {
        self.rag_data_dir().join("uploads")
    }
    ...
    pub fn rag_stdout_log(&self) -> PathBuf {
        self.logs_dir.join("rag.stdout.log")
    }

    pub fn rag_stderr_log(&self) -> PathBuf {
        self.logs_dir.join("rag.stderr.log")
    }
    ...
    pub fn is_installed(&self) -> bool {
        self.install_state_path.exists()
            ...
            && self.installed_rag_app_main().exists()
    }
    ...
    pub fn current_rag_url(&self) -> String {
        self.load_install_state()
            .map(|state| state.rag_url)
    ...
    pub fn load_rag_pid_record(&self) -> Option<EnginePidRecord> {
        read_json_file(&self.rag_pid_path).ok()
    }

    pub fn save_rag_pid_record(&self, record: &EnginePidRecord) -> AppResult<()> {
        write_json_file(&self.rag_pid_path, record)
    }

    pub fn clear_rag_pid_record(&self) -> AppResult<()> {
        if self.rag_pid_path.exists() {
            fs::remove_file(&self.rag_pid_path).map_err(|error| {
                format!(
                    "Could not remove RAG PID file `{}`: {error}",
                    self.rag_pid_path.display()
                )
            })?;
        }
    ...
    pub fn write_engine_env(&self, manifest: &InstallerManifest) -> AppResult<()> {
        ...
        let rag_data_dir = self.rag_data_dir();
        let contents = format!(
    ...
    pub fn load_engine_env_pairs(&self) -> AppResult<Vec<(String, String)>> {
        ...
        pairs.push(("RAG_DATA_DIR".to_string(), self.rag_data_dir().display().to_string()));
        pairs.push(("AEGIS_RAG_URL".to_string(), self.current_rag_url()));
        pairs.push((
...
impl InstallerManifest {
    pub fn normalized(mut self) -> Self {
        if self.rag_host.trim().is_empty() {
            self.rag_host = DEFAULT_RAG_HOST.to_string();
        }
        if self.rag_port == 0 {
            self.rag_port = DEFAULT_RAG_PORT;
        }
        if self.rag_url.trim().is_empty() {
            self.rag_url = format!("http://{}:{}", self.rag_host, self.rag_port);
        }
...
fn default_rag_host_string() -> String {
    DEFAULT_RAG_HOST.to_string()
}

fn default_rag_port() -> u16 {
    DEFAULT_RAG_PORT
}

fn default_rag_url_string() -> String {
    DEFAULT_RAG_URL.to_string()
}
