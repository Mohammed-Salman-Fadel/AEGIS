...
//! Next TODOs: add runtime updates/rollback commands and signed artifact verification.

use std::env;
use std::fs;
use std::io::{self, copy, IsTerminal, Write};
...
use crate::runtime::{
    ...
    DEFAULT_ENGINE_HOST, DEFAULT_ENGINE_PORT, DEFAULT_MODEL, DEFAULT_RAG_HOST, DEFAULT_RAG_PORT,
    DEFAULT_UI_URL,
};
use crate::runner;
use crate::ui::Ui;
...
pub struct RuntimeStatus {
    ...
    pub rag_url: String,
    pub rag_pid: Option<u32>,
    pub rag_running: bool,
...
#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub struct RuntimeStartStatus {
    pub ollama_ready: bool,
    pub rag: StartOutcome,
    pub engine: StartOutcome,
}
    ...
    InstallPlan {
        ...
        steps: vec![
            ...
            InstallStep {
                ...
                description:
                    "Downloads the runtime zip, verifies SHA-256, extracts it to a staged directory, and promotes it only after validation."
                        .to_string(),
                verification_hint:
                    "The staged runtime must contain bin/aegis.exe, bin/aegis-engine.exe, ui/index.html, rag/app/main.py, python/python.exe, config/default.env, and version.txt."
                        .to_string(),
            },
            InstallStep {
            ...
            InstallStep {
                ...
                platform: "Windows x64".to_string(),
                description:
                    "Installs Ollama if approved and missing, starts the local Ollama service when needed, and ensures the manifest default model is present."
                        .to_string(),
                verification_hint: format!("Ollama /api/tags should respond and include {DEFAULT_MODEL}.")
                    .to_string(),
            },
            InstallStep {
                name: "Prepare Python RAG sidecar".to_string(),
                platform: "Windows x64".to_string(),
                description:
                    "Creates the local RAG virtual environment from the bundled Python runtime and offline wheelhouse."
                        .to_string(),
                verification_hint:
                    "The RAG venv Python should exist and the sidecar should answer /health and /init."
                    .to_string(),
            },
            InstallStep {
                name: "Write config and start localhost runtime".to_string(),
                platform: "Windows x64".to_string(),
                description:
                    "Writes engine.env, launches RAG and aegis-engine.exe in the background, waits for /health, and opens the browser."
                        .to_string(),
                verification_hint:
...
pub fn runtime_status(layout: &RuntimeLayout) -> RuntimeStatus {
    ...
    let rag_url = install_state
        .as_ref()
        .map(|state| state.rag_url.clone())
        .unwrap_or_else(|| format!("http://{DEFAULT_RAG_HOST}:{DEFAULT_RAG_PORT}"));
    ...
    let rag_pid_record = layout.load_rag_pid_record();
    let rag_pid_running = rag_pid_record
        .as_ref()
        .map(|record| runner::is_process_running(record.pid))
        .unwrap_or(false);
    let rag_health = url_reachable(&format!("{rag_url}/health"));
    ...
    RuntimeStatus {
        ...
        rag_url,
        rag_pid: rag_pid_record.as_ref().map(|record| record.pid),
        rag_running: rag_health || rag_pid_running,
...
pub fn start_runtime_stack(layout: &RuntimeLayout) -> AppResult<RuntimeStartStatus> {
    if !layout.is_installed() {
        return Err(format!(
            "AEGIS is not installed in `{}` yet. Run `aegis install` first.",
            layout.root.display()
    ...
    let ollama_exe = find_ollama_exe().ok_or_else(|| {
        "Ollama is not installed or could not be located. Run `aegis install` to repair the runtime."
            .to_string()
    })?;
    if !ollama_reachable() {
    ...
    let rag = start_rag(layout)?;
    let engine = start_engine(layout)?;
    let status = runtime_status(layout);
    wait_for_url(&status.ui_url, Duration::from_secs(20))?;

    Ok(RuntimeStartStatus {
        ollama_ready: true,
        rag,
        engine,
    })
...
pub fn ensure_runtime_started_silent(layout: &RuntimeLayout) -> AppResult<()> {
    if !layout.is_installed() {
        return Ok(());
    }

    let _ = start_runtime_stack(layout)?;
    Ok(())
}
...
pub fn stop_runtime_stack(layout: &RuntimeLayout) -> AppResult<(bool, bool)> {
    let engine_stopped = stop_engine(layout)?;
    let rag_stopped = stop_rag(layout)?;
    Ok((engine_stopped, rag_stopped))
}

pub fn start_rag(layout: &RuntimeLayout) -> AppResult<StartOutcome> {
    if !layout.is_installed() {
        return Err(format!(
            "AEGIS is not installed in `{}` yet. Run `aegis install` first.",
            layout.root.display()
    ...
    let status = runtime_status(layout);
    let (rag_host, rag_port) = layout
        .load_install_state()
        .map(|state| (state.rag_host, state.rag_port))
        .unwrap_or_else(|| (DEFAULT_RAG_HOST.to_string(), DEFAULT_RAG_PORT));
    if status.rag_running {
        initialize_rag(&status.rag_url)?;
        return Ok(StartOutcome::AlreadyRunning);
    }

    ensure_rag_runtime(layout)?;

    let program = layout.rag_venv_python_exe();
    if !program.exists() {
        return Err(format!(
            "The RAG Python runtime was not found at `{}`.",
            program.display()
        ));
    }
    ...
    let env_pairs = vec![
        (
            "RAG_DATA_DIR".to_string(),
            layout.rag_data_dir().display().to_string(),
        ),
        (
            "AEGIS_RAG_EMBEDDING_MODEL".to_string(),
            layout
                .installed_rag_dir()
                .join("models")
    ...
    let pid = runner::spawn_detached_process(
        &program,
        &args,
        &layout.installed_rag_dir(),
        &env_pairs,
        &layout.rag_stdout_log(),
        &layout.rag_stderr_log(),
    )?;
    ...
    let record = EnginePidRecord {
        ...
        version: layout
            ...
            .map(|state| state.version)
            .unwrap_or_else(|| "unknown".to_string()),
        url: status.rag_url.clone(),
    };
    layout.save_rag_pid_record(&record)?;

    wait_for_url(&format!("{}/health", status.rag_url), Duration::from_secs(30))?;
    initialize_rag(&status.rag_url)?;

    Ok(StartOutcome::Started)
}

pub fn stop_rag(layout: &RuntimeLayout) -> AppResult<bool> {
    let Some(record) = layout.load_rag_pid_record() else {
        return Ok(false);
    };

    if !runner::is_process_running(record.pid) {
        layout.clear_rag_pid_record()?;
        return Ok(false);
    }
...
fn install_runtime(ctx: &AppContext, args: &InstallArgs) -> AppResult<RuntimeStatus> {
    ...
    let manifest = fetch_manifest(&manifest_source)?.normalized();
    ...
    ensure_rag_runtime(&ctx.runtime)?;
    println!("{}", ctx.ui.success("RAG runtime verified."));

    let ollama_exe = ensure_ollama_installed_and_running(&ctx.runtime, &manifest, args.yes)?;
    ...
    let install_state = InstallState {
        ...
        rag_host: manifest.rag_host.clone(),
        rag_port: manifest.rag_port,
        rag_url: manifest.rag_url.clone(),
    };
    ...
    if ctx.runtime.load_rag_pid_record().is_some() {
        let _ = stop_rag(&ctx.runtime);
    }

    let started = start_runtime_stack(&ctx.runtime)?;
    match started.rag {
        StartOutcome::Started => println!("{}", ctx.ui.success("AEGIS RAG sidecar started.")),
        StartOutcome::AlreadyRunning => println!("{}", ctx.ui.muted("AEGIS RAG sidecar was already running.")),
    }
    match started.engine {
        StartOutcome::Started => println!("{}", ctx.ui.success("AEGIS engine started.")),
        StartOutcome::AlreadyRunning => println!("{}", ctx.ui.muted("AEGIS engine was already running.")),
    }
...
fn validate_runtime_bundle(stage_dir: &Path) -> AppResult<()> {
    let required = [
        ...
        stage_dir.join("rag").join("app").join("main.py"),
        stage_dir.join("rag").join("requirements.txt"),
        stage_dir.join("rag").join("wheels"),
        stage_dir.join("python").join("python.exe"),
...
fn ensure_ollama_installed_and_running(
    ...
    if !assume_yes && !confirm_ollama_install()? {
        return Err(
            "Ollama is required for local inference. Re-run `aegis install --yes` or install Ollama manually, then run `aegis install` again."
                .to_string(),
        );
    }
...
fn confirm_ollama_install() -> AppResult<bool> {
    if !io::stdin().is_terminal() {
        return Ok(false);
    }

    print!("Ollama is required but was not found. Install it now? [Y/n] ");
    io::stdout()
        .flush()
        .map_err(|error| format!("Could not flush installer prompt: {error}"))?;

    let mut answer = String::new();
    io::stdin()
        .read_line(&mut answer)
        .map_err(|error| format!("Could not read installer prompt response: {error}"))?;

    let answer = answer.trim().to_lowercase();
    Ok(answer.is_empty() || answer == "y" || answer == "yes")
}

fn ensure_rag_runtime(layout: &RuntimeLayout) -> AppResult<()> {
    fs::create_dir_all(layout.rag_data_dir()).map_err(|error| {
        format!(
            "Could not create RAG data directory `{}`: {error}",
            layout.rag_data_dir().display()
        )
    })?;
    fs::create_dir_all(layout.rag_upload_dir()).map_err(|error| {
        format!(
            "Could not create RAG upload directory `{}`: {error}",
            layout.rag_upload_dir().display()
        )
    })?;

    if layout.rag_venv_python_exe().exists() {
        return Ok(());
    }

    let bundled_python = layout.bundled_python_exe();
    if !bundled_python.exists() {
        return Err(format!(
            "The bundled Python runtime is missing at `{}`.",
            bundled_python.display()
        ));
    }

    if !layout.installed_rag_requirements().exists() {
        return Err(format!(
            "The RAG requirements file is missing at `{}`.",
            layout.installed_rag_requirements().display()
        ));
    }

    if !layout.installed_rag_wheelhouse_dir().is_dir() {
        return Err(format!(
            "The offline RAG wheelhouse is missing at `{}`.",
            layout.installed_rag_wheelhouse_dir().display()
        ));
    }

    if layout.rag_venv_dir.exists() {
        fs::remove_dir_all(&layout.rag_venv_dir).map_err(|error| {
            format!(
                "Could not reset RAG virtual environment `{}`: {error}",
                layout.rag_venv_dir.display()
            )
        })?;
    }
    ...
    let venv_args = vec![
        "-m".to_string(),
        "venv".to_string(),
        layout.rag_venv_dir.display().to_string(),
    ];
    runner::run_program_and_wait(
    ...
    let pip_args = vec![
        ...
        layout.installed_rag_wheelhouse_dir().display().to_string(),
        "-r".to_string(),
        layout.installed_rag_requirements().display().to_string(),
    ];
    runner::run_program_and_wait(
        &layout.rag_venv_python_exe().display().to_string(),
        &pip_args,
        &layout.current_runtime_dir,
    )
}

fn initialize_rag(rag_url: &str) -> AppResult<()> {
    let init_url = format!("{}/init", rag_url.trim_end_matches('/'));
    Client::new()
        .post(&init_url)
        .json(&json!({}))
        .send()
        .map_err(|error| format!("Could not initialize RAG at `{init_url}`: {error}"))?
        .error_for_status()
        .map_err(|error| format!("RAG init failed at `{init_url}`: {error}"))?;
